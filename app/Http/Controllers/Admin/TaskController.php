...
class TaskController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        return view('admin.tasks.index', compact('tasks', 'search', 'status', 'statuses', 'allSupplies'));
    }
    ...
    {
        Log::info('Task update request:', $request->all());
        $data = $request->validated();
    ...
    public function approve(Task $task, Request $request): RedirectResponse|JsonResponse
    {
        ...
        if (in_array($task->status, ['approved', 'cancelled'])) {
            ...
            return redirect()->route('admin.tasks.index')->with('error', $message);
        }

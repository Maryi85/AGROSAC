...
<div class="flex items-center justify-between">
    <h2 class="text-lg font-semibold text-emerald-700">Crear Nueva Tarea</h2>
        ...
        <form method="POST" action="{{ route('foreman.tasks.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="scheduled_for" id="scheduled_for" required 
                           value="{{ old('scheduled_for', date('Y-m-d')) }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required 
                          ...
                          class="w-full border border-emerald-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>

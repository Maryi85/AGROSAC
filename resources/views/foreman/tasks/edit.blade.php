                    ...
                    <select name="crop_id" id="crop_id" class="w-full border border-emerald-200 rounded px-3 py-2 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        @foreach($crops as $crop)
                            <option value="{{ $crop->id }}" 
                                    ...
                                    {{ old('crop_id', $task->crop_id) == $crop->id ? 'selected' : '' }}>
                                {{ $crop->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @php
                    ...
                    } elseif ($task->hours > 0 && floatval($task->hours) % 8 === 0 && floatval((int)$task->price_per_day) > 0) {
        ...
        <div id="payment-fields" class="space-y-4">
            ...
            <div class="bg-emerald-50 border border-emerald-200 rounded p-4">
                ...
                <div class="relative">
                    ...
                    <input type="hidden" name="total_payment" id="total_payment" value="{{ $task->total_payment }}">
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function openTask(task) {
        ...
        this.showModal = true;
        // Reinicializar Lucide para los iconos en el modal
        setTimeout(() => {
    ...
    function togglePaymentFields() {
        ...
        if (hoursField) hoursField.classList.add('hidden');
    ...
    function calculateTotal() {
        ...
        if (selectedType === 'hours') {
            ...
            const p = parseFloat(document.getElementById('price_per_hour').value) || 0;
            total = h * p;
        } else if (selectedType === 'days') {
            const d = parseFloat(document.getElementById('days').value) || 0;
            const p = parseFloat(document.getElementById('price_per_day').value) || 0;
            total = d * p;
        ...
        try {
            ...
            if (data.success) {
                ...
                data.workers.forEach(worker => {
                    ...
                    if (currentValue == worker.id) option.selected = true;
    ...
    async function loadCrops() {
        ...
        if (cropsLoading) cropsLoading.classList.remove('hidden');
        ...
        try {
            ...
            if (data.success) {
                ...
                data.crops.forEach(crop => {
                    ...
                    if (crop.plot_id) option.setAttribute('data-plot-id', crop.plot_id);
                    if (currentValue == crop.id) option.selected = true;
            ...
            if (cropsLoading) cropsLoading.classList.add('hidden');
    ...
    if (refreshWorkersBtn) refreshWorkersBtn.addEventListener('click', refreshWorkers);
    if (refreshCropsBtn) refreshCropsBtn.addEventListener('click', loadCrops);

    // Listeners para cálculos en tiempo real
    ['hours', 'price_per_hour', 'days', 'price_per_day', 'kilos', 'price_per_kg'].forEach(id => {
    ...
    if (cropsSelect && plotSelect) {
        ...
        plotSelect.addEventListener('change', function() {
            ...
            if (cropsForPlot.length === 1) {
                ...
                if (currentCropPlotId !== selectedPlotId) cropsSelect.value = '';
            }
        });
    }
    ...
    function calculateTotal() {
        ...
        let total = 0;
        if (selectedType === 'hours') total = hours * pricePerHour;
        else if (selectedType === 'days') total = days * pricePerDay;
        else if (selectedType === 'quantity') total = kilos * pricePerKg;
        
        if (totalInput) totalInput.value = total.toFixed(0);
        if (totalDisplay) {
            totalDisplay.value = '$' + new Intl.NumberFormat('es-CO', {
                minimumFractionDigits: 0,
                maximumFractionDigits: 0
            }).format(total);
        }

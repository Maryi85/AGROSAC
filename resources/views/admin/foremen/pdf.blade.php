...
<head>
    <meta charset="utf-8">
    <title>Reporte de Mayordomos - AGROSAC</title>
    <style>
        @page {
            margin: 10mm 15mm;
        }
        ...
        <table class="header-table">
            <tr>
                ...
                <td style="text-align: right; vertical-align: middle;">
                    <div style="font-size: 14pt; font-weight: bold; color: #333;">REPORTE DE MAYORDOMOS</div>
            ...
            <td style="vertical-align: top; width: 40%;">
                <div class="summary-box" style="width: 100%;">
                    ...
                    <div class="summary-row">
                        <span class="summary-label">Total Mayordomos:</span>
                        <span class="summary-value">{{ $foremen->count() }}</span>
                    </div>
                </div>
            </td>
        </tr>
    ...
    <table class="data-table">
        ...
        <tbody>
            @forelse($foremen as $foreman)
                <tr>
                    <td>{{ $foreman->id }}</td>
                    <td class="font-bold">{{ $foreman->name }}</td>
                    <td>{{ $foreman->email }}</td>
                    <td class="text-center">
                        <span class="badge {{ $foreman->email_verified_at ? 'badge-active' : 'badge-inactive' }}">
                            {{ $foreman->email_verified_at ? 'Activo' : 'Inactivo' }}
                        </span>
                    </td>
                    <td class="text-right">{{ $foreman->created_at->format('d/m/Y') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="5" style="text-align: center; padding: 20px;">No hay mayordomos registrados</td>
                </tr>
            @endforelse
        </tbody>
    </table>

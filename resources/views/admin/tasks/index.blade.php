                        ...
                        <td class="py-3 pr-4 text-right">
                            <div class="flex items-center gap-1 justify-end">
                                ...
                                @if(in_array($task->status, ['pending', 'in_progress']))
                                    <a href="{{ route('admin.tasks.edit', $task) }}" class="inline-flex items-center gap-1 px-2 py-1 border rounded hover:bg-emerald-50 text-emerald-600 shadow-sm transition-all" title="Editar">
                                ...
                                @if($task->status === 'completed')
                                    <form method="POST" action="{{ route('admin.tasks.approve', $task) }}" class="ajax-task-form inline" data-confirm="true" data-message="¿Aprobar esta tarea?">
                                        @csrf
                                ...
                                <!-- Invalidar -->
                                @if($task->status === 'completed')
                                    <form method="POST" action="{{ route('admin.tasks.invalidate', $task) }}" class="ajax-task-form inline" data-confirm="true" data-message="¿Marcar esta tarea como inválida?">
                                        @csrf
                                ...
                                <!-- Eliminar -->
                                @if(in_array($task->status, ['pending', 'in_progress']))
                                    <form method="POST" action="{{ route('admin.tasks.destroy', $task) }}" class="ajax-task-form inline" data-confirm="true" data-message="¿Eliminar esta tarea?">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="grid grid-cols-1 gap-4 md:hidden">
            ...
            <div id="task-card-{{ $task->id }}" class="bg-white border rounded-lg p-4 shadow-sm relative">
                ...
                <div class="flex items-center justify-end gap-3 border-t pt-3">
                    ...
                    @if(in_array($task->status, ['pending', 'in_progress']))
                        <a href="{{ route('admin.tasks.edit', $task) }}" class="p-2 text-emerald-600 bg-emerald-50 rounded-lg shadow-sm" title="Editar">

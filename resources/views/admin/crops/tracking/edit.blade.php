...
<div class="flex items-center justify-between">
    <h2 class="text-lg font-semibold text-emerald-700">Editar Seguimiento de Cultivo</h2>
...
<div class="bg-white border rounded p-6">
    ...
    <form method="POST" action="{{ route('admin.crop-tracking.update', $trackingModel) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="date" 
                       id="tracking_date" 
                       name="tracking_date" 
                       value="{{ old('tracking_date', $trackingModel->tracking_date->format('Y-m-d')) }}"
                ...
                <select id="crop_id" 
                    ...
                    @foreach($crops as $crop)
                        <option value="{{ $crop->id }}" 
                                ...
                                {{ old('crop_id', $trackingModel->crop_id) == $crop->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select id="phase" 
                    ...
                    @foreach($phases as $key => $label)
                        <option value="{{ $key }}" {{ old('phase', $trackingModel->phase) == $key ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <input type="date" 
                   id="cut_date" 
                   name="cut_date" 
                   value="{{ old('cut_date', $trackingModel->cut_date?->format('Y-m-d')) }}"
                   class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
        </div>
        ...
        <div>
            ...
            <textarea id="activities" 
                      name="activities" 
                      rows="10"
                      class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 font-mono text-sm"
                      placeholder="Las actividades se generan automáticamente basadas en las tareas realizadas por trabajadores y los insumos agregados a este cultivo...">{{ old('activities', $activitiesText ?? $trackingModel->activities) }}</textarea>
            <p class="text-xs text-gray-500 mt-1">Este campo se llena automáticamente con las tareas realizadas por trabajadores y los insumos agregados al cultivo seleccionado. Puede editarlo manualmente si lo desea.</p>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    cropSelect.addEventListener('change', updatePlot);
    
    // Inicializar con el valor actual del seguimiento
    const currentPlotId = @json($trackingModel->plot_id);
    if (currentPlotId && plots[currentPlotId]) {
        const plot = plots[currentPlotId];
        plotSelect.innerHTML = `<option value="${plot.id}" selected>${plot.name} - ${plot.area} ha</option>`;
    } else {
        // Si no se encuentra, intentar actualizar desde el cultivo seleccionado
        updatePlot();
    }

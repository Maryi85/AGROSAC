        ...
        <table class="header-table">
            <tr>
                ...
                <td style="text-align: right; vertical-align: middle;">
                    <div style="font-size: 14pt; font-weight: bold; color: #333;">REPORTE DE PRÉSTAMOS</div>
            ...
            <td style="vertical-align: top; width: 40%;">
                <div class="summary-box" style="width: 100%;">
                    ...
                    <div class="summary-row">
                        ...
                        <span class="summary-value">{{ $loans->count() }}</span>
                    </div>
                </div>
            </td>
        </tr>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th style="width: 15%;">Tarea/Lote</th>
        ...
        <tbody>
            @forelse($loans as $loan)
                <tr>
                    <td>{{ $loan->id }}</td>
                    <td class="font-bold">{{ $loan->tool->name ?? 'N/A' }}</td>
                    <td>{{ $loan->user->name ?? 'N/A' }}</td>
                    <td style="font-size: 8pt; color: #4b5563;">
                        @if($loan->task)
                            {{ $loan->task->description }} 
                            <br>
                            <small>({{ $loan->task->plot->name ?? 'Lote General' }})</small>
                        @else
                            —
                        @endif
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="8" style="text-align: center; padding: 20px;">No hay préstamos registrados</td>
                </tr>
            @endforelse
        </tbody>
    </table>

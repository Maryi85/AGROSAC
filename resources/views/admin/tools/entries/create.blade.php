...
@section('title', 'Nueva Entrada de Herramienta')
...
@section('content')
<form method="POST" action="{{ route('admin.tool-entries.store') }}" class="space-y-6">
    @csrf
    
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-medium text-gray-900">Información de la Entrada</h3>
            <a href="{{ route('admin.tool-entries.index') }}" class="inline-flex items-center gap-2 px-4 py-2 border border-emerald-300 rounded text-emerald-700 hover:bg-emerald-100 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" id="notes" rows="3" maxlength="1000"
                          ...
                          placeholder="Información adicional sobre la entrada...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="flex justify-end gap-4 pt-6 border-t">
        <a href="{{ route('admin.tool-entries.index') }}" 
